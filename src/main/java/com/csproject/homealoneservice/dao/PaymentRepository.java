...
//  List<PaymentEntity> findAllByAndInstallmentBetween();
  @Query("select house.houseName,function('date_format',payment.installment,'%M') as Da ,SUM(payment.payHouseAmount)from PaymentEntity payment,HouseEntity house,RentingHouseEntity renting_house,ManagerEntity manager where manager.mid = ?1 AND house.mid = ?1 AND renting_house.hid = house.hid AND payment.rid = renting_house.rid AND payment.payHouseStatus = ?2 AND payment.installment BETWEEN  ?3  And  ?4 GROUP BY house.houseName,function('date_format',payment.installment,'%M')")
